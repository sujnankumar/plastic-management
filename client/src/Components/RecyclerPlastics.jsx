...
const PlasticsPage = () => {
    const [recyclerPlasticsData, setRecyclerPlasticsData] = useState([]);
    const [recycledPlasticsData, setRecycledPlasticsData] = useState([]);
    const [recyclerCountData, setRecyclerCountData] = useState([]);
    const [recycledCountData, setRecycledCountData] = useState([]);
        ...
        const fetchRecyclerAndPlastics = async () => {
            if (userID) {
                ...
                try {
                    ...
                    setRecyclerCountData(plasticsResponse.data.recycler_count_data);
                    setRecycledCountData(plasticsResponse.data.recycled_count_data);
                    setRecyclerPlasticsData(plasticsResponse.data.recycler_plastics_data);
                    setRecycledPlasticsData(plasticsResponse.data.recycled_plastics_data);
                    setError(null);
                } catch (error) {
    ...
    return (
        <div className="container mx-auto p-4 sm:p-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mb-6 border border-green-300">
            <h3 className="text-2xl font-semibold mb-4 text-green-700">Collected Plastics Details</h3>
                <table className="min-w-full text-md bg-white rounded-lg shadow-md mb-6 border border-green-300">
                    ...
                    <tbody>
                        {recyclerCountData.length > 0 ? (
                            recyclerCountData.map((plastic) => (
                ...
                <table className="min-w-full text-md bg-white rounded-lg mb-6 shadow-md border border-green-300">
                    <thead className="bg-green-100">
                        <tr className="text-left">
                            <th className="p-2 text-green-700 text-center">ID #</th>
                            <th className="p-2 text-green-700 text-center">Collected Date</th>
                            <th className="p-2 text-green-700 text-center">Type</th>
                            <th className="p-2 text-green-700 text-center">Cost</th>
                        </tr>
                    </thead>
                    <tbody>
                        {recyclerPlasticsData.length > 0 ? (
                            recyclerPlasticsData.map((plastic) => (
                                <tr key={plastic.id} className="border-b border-green-200">
                                    <td className="p-2 text-center">{plastic.id}</td>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mb-6 border border-green-300">
                <h3 className="text-2xl font-semibold mb-4 text-green-700">Recycled Plastics Details</h3>
                ...
                <table className="min-w-full text-md bg-white rounded-lg shadow-md mb-6 border border-green-300">
                    ...
                    <tbody>
                        {recycledCountData.length > 0 ? (
                            recycledCountData.map((plastic) => (
                                <tr key={plastic.id} className="border-b border-green-200">
                                    <td className="p-2 text-center">{plastic.id}</td>
                                    <td className="p-2 text-center">{plastic.name}</td>
                                    <td className="p-2 text-center">{plastic.count}</td>
                ...
                <table className="min-w-full text-md bg-white rounded-lg shadow-md border border-green-300">
                    ...
                    <tbody>
                        {recycledPlasticsData.length > 0 ? (
                            recycledPlasticsData.map((plastic) => (
                                <tr key={plastic.id} className="border-b border-green-200">
                                    <td className="p-2 text-center">{plastic.id}</td>
                                    <td className="p-2 text-center">{new Date(plastic.recycled_date).toLocaleString()}</td>
                                    <td className="p-2 text-center">{plastic.type}</td>
